...
type Snap = Pick<Project, 'tracks' | 'clips' | 'markers' | 'inF' | 'outF' | 'media' | 'w' | 'h' | 'rate' | 'drop' | 'name' | 'master' | 'sottotitoli' | 'sequenze' | 'seqAttiva'> & { label: string };
...
class Store {
  ...
  private snap(label: string): Snap {
    const d = this.doc;
    return structuredClone({ tracks: d.tracks, clips: d.clips, markers: d.markers, inF: d.inF, outF: d.outF, media: d.media, w: d.w, h: d.h, rate: d.rate, drop: d.drop, name: d.name, master: d.master, sottotitoli: d.sottotitoli, sequenze: d.sequenze, seqAttiva: d.seqAttiva, label });
  }
